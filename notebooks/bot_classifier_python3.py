...
def classify_bot_or_not(in_df):
    ...
    predict_prob = classifier.predict_proba(test_features)
    
    return (prediction, predict_prob[0][1])

if __name__ == '__main__':
    in_df = retrieve_user_info(sys.argv[1])
    prediction,prob = classify_bot_or_not(in_df)
    in_df['bot_or_not'] = prediction
    in_df['bot_prob'] = prob
